...
水果小丑牌是一款基于"小丑牌"(Balatro) 核心机制设计的 H5 休闲益智小游戏。玩家在 16×25 的水果矩阵中，通过选择 3×3 区域来组合分数与倍率，策略性地完成关卡目标。

### 1.2 核心特色

- **策略选择**: 3×3 区域选择 + 4 次选择机会 = 丰富的策略组合
...
```
开始游戏 → 选择关卡 → 进入矩阵 → 选择3×3区域(最多4次) → 计算得分 → 判定通关
    ↓                                                              ↓
    ← ← ← ← ← ← ← ← ← ← 未通过（可重试）← ← ← ← ← ← ← ← ← ←
    ↓                                                              ↓
  通关 → 进入商店（每关结束后）→ 下一关
...
| 🍌 香蕉 | 黄色弯香蕉 | 倍率型 | 增加倍率 | 倍率 +10 |
| 🍒 车厘子 | 红色双樱桃 | 倍率型 | 乘算倍率 | 倍率 ×3 |
...
- 玩家使用 **3×3 正方形框** 在 16×25 矩阵中选择区域
- 每关共有 **4 次选择机会**
- 3×3 框可放置在矩阵中任意合法位置（左上角坐标范围：行 0~13，列 0~22）
...
- 若玩家使用 **少于 4 次**选择即达到目标分数，剩余次数**转化为金币**
- 转化规则：**每剩余 1 次选择 = 2 枚金币**
- 示例：目标 200 分，第 2 次选择后已达 250 分 → 剩余 2 次 → 获得 4 枚金币
...
乘法倍率 = 3
最终倍率 = (1 + 20) × 3 = 63
单次选择得分 = 105 × 63 = 6,615 分
```

#### 2.4.3 关卡总分

```
关卡总分 = 第1次选择得分 + 第2次选择得分 + 第3次选择得分 + 第4次选择得分
```
...
采用指数递增公式：`目标分数 = base × (growth_rate ^ (关卡序号 - 1))`

| 大关 | 小关 | 关卡编号 | 目标分数 | 可用水果 | 说明 |
|------|------|---------|---------|---------|------|
| 1 | 1-1 | #1 | 150 | 苹果、香蕉、车厘子 | 入门关 |
| 1 | 1-2 | #2 | 200 | 苹果、香蕉、车厘子 | +1 随机限制 |
| 1 | 1-3 | #3 | 280 | 苹果、香蕉、车厘子 | +2 随机限制 |
| 2 | 2-1 | #4 | 400 | + 葡萄 | 新水果登场 |
| 2 | 2-2 | #5 | 550 | + 葡萄 | +1 随机限制 |
| 2 | 2-3 | #6 | 750 | + 葡萄 | +2 随机限制 |
| 3 | 3-1 | #7 | 1,050 | + 橙子、柠檬 | 新水果登场 |
| 3 | 3-2 | #8 | 1,450 | + 橙子、柠檬 | +1 随机限制 |
| 3 | 3-3 | #9 | 2,000 | + 橙子、柠檬 | +2 随机限制 |
| 4 | 4-1 | #10 | 2,800 | + 猕猴桃 | 新水果登场 |
| 4 | 4-2 | #11 | 3,850 | + 猕猴桃 | +1 随机限制 |
| 4 | 4-3 | #12 | 5,300 | + 猕猴桃 | +2 随机限制 |
| 5 | 5-1 | #13 | 7,300 | + 西瓜 | 新水果登场 |
| 5 | 5-2 | #14 | 10,000 | + 西瓜 | +1 随机限制 |
| 5 | 5-3 | #15 | 13,800 | + 西瓜 | +2 随机限制 |
| 6 | 6-1 | #16 | 19,000 | + 草莓、桃子 | 新水果登场 |
| 6 | 6-2 | #17 | 26,000 | + 草莓、桃子 | +1 随机限制 |
| 6 | 6-3 | #18 | 36,000 | + 草莓、桃子 | +2 随机限制 |
| 7 | 7-1 | #19 | 50,000 | 全部水果 | 终极挑战开始 |
| 7 | 7-2 | #20 | 68,000 | 全部水果 | +1 随机限制 |
| 7 | 7-3 | #21 | 94,000 | 全部水果 | +2 随机限制 |
| 8 | 8-1 | #22 | 130,000 | 全部水果 | 最终章 |
| 8 | 8-2 | #23 | 178,000 | 全部水果 | +1 随机限制 |
| 8 | 8-3 | #24 | 245,000 | 全部水果 | +2 随机限制，Boss 关 |
...
**保证机制**：
- 每个矩阵至少包含 3 个车厘子（确保倍率×3 可用）
- 每个矩阵至少包含 5 个香蕉（确保倍率+可用）
- 矩阵生成后进行"可通关性"校验（见第 10 节核心算法）
...
| 🍌 香蕉强化 | 10 金币 | 香蕉倍率 +5 | 5 次 | +10→+15→+20→+25→+30→+35 |
| 🍒 车厘子强化 | 12 金币 | 车厘子倍率 ×0.5（叠加） | 3 次 | ×3→×4.5→×6→×7.5 |
...
| 商品 | 价格 | 效果 | 最大购买次数 | 说明 |
|------|------|------|-------------|------|
| 🍀 苹果幸运草 | 6 金币 | 苹果出现概率 +5% | 3 次 | 55%→60%→65%→70% |
| 🍀 香蕉幸运草 | 8 金币 | 香蕉出现概率 +5% | 3 次 | 25%→30%→35%→40% |
| 🍀 车厘子幸运草 | 8 金币 | 车厘子出现概率 +5% | 3 次 | 20%→25%→30%→35% |
...
| `ONE_CHANCE` | 一击必杀 | 仅允许 1 次选择机会（替代 4 次） | ⭐⭐⭐⭐⭐ | 第 8 大关起 |
...
interface GameStore {
  ...
  // 选择次数
  maxSelections: number;       // 最大选择次数 (默认4)
...
enum FruitType {
  APPLE = 'apple',           // 苹果 +10
  BANANA = 'banana',         // 香蕉 倍率+10
  CHERRY = 'cherry',         // 车厘子 倍率×3
  GRAPE = 'grape',           // 葡萄 +15
  ORANGE = 'orange',         // 橙子 +25
...
function generateMatrix(
  ...
  for (let row = 0; row < 16; row++) {
    matrix[row] = [];
    for (let col = 0; col < 25; col++) {
      matrix[row][col] = weightedRandom(config.fruitProbabilities);
    }
  }

  // 2. 保证最少车厘子和香蕉数量
  ensureMinimumCount(matrix, FruitType.CHERRY, 3);
  ensureMinimumCount(matrix, FruitType.BANANA, 5);

  // 3. 应用限制效果
  for (const restriction of config.restrictions) {
    matrix = restriction.apply(matrix);
  }

  // 4. 可通关性校验
  if (!isSolvable(matrix, config.targetScore, config.maxSelections, upgrades)) {
    // 重新生成（最多重试 10 次）
    return generateMatrix(config, upgrades); // 递归重试
  }
...
function greedyBestScore(
  ...
  const used = Array.from({ length: 16 }, () => Array(25).fill(false));
  let totalScore = 0;
